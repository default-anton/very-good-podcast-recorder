...
func TestNewHandlerHealthz(t *testing.T) {
	logger := logging.NewLogger(nil, "sessiond", logging.KindService, slog.LevelInfo)
	...
	if got, want := payload["service"], "sessiond"; got != want {
...
func TestRunDoesNotLogListeningOnBindFailure(t *testing.T) {
	...
	logger := logging.NewLogger(&buffer, "sessiond", logging.KindService, slog.LevelInfo)

	err = Run(context.Background(), Config{Addr: listener.Addr().String(), Version: "dev"}, logger)
	if err == nil {
		t.Fatal("expected bind failure")
	}
	if !strings.Contains(err.Error(), "listen sessiond") {
		t.Fatalf("error = %q, want listen sessiond context", err)
	}
	if strings.Contains(buffer.String(), "sessiond listening") {
...
func TestRunLogsBoundAddrWhenUsingEphemeralPort(t *testing.T) {
	...
	for {
		logs := buffer.String()
		if strings.Contains(logs, "sessiond listening") {
			break
		}
	...
	if !strings.Contains(logs, `"msg":"sessiond stopped"`) {

...
func Run(ctx context.Context, cfg Config, logger *slog.Logger) error {
	...
	if err != nil {
		return fmt.Errorf("listen sessiond: %w", err)
	}
	...
	logger.Info("sessiond listening", slog.String("addr", boundAddr))
	err = server.Serve(listener)
	if err != nil && err != http.ErrServerClosed {
		return fmt.Errorf("serve sessiond: %w", err)
	}
	...
	logger.Info("sessiond stopped", slog.String("addr", boundAddr))
	return nil
}
